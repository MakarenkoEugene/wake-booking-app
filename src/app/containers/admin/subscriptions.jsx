...
import { SubscriptionsForm, Form, LineBlock } from '@components';
import { findChanges, formValidate } from '@utils';
import { Button } from '@components/ui';

import AddIcon from '@material-ui/icons/Add';

const Subscriptions = ({ rootStore: { ui, user } }) => {
  ...
  const create = async (event) => {
    ...
    ui.setLoading('root', true);
    const res = await ui.makeRequest('post', 'subscriptions', req);
    ui.setLoading('root', false);

    if (!res) return;

    user.setParks(user.parks.map((park) => (
      park._id === parkId ? { ...park, subscriptions: [...park.subscriptions, res.data] } : park)));
  ...
  return (
    <Grid container direction='column' className='loader'>
      { close && user.activePark?.subscriptions.map((sub) => (
        <LineBlock
          key={sub._id}
          onClick={() => openForm(sub._id)}
          sub={{ ...sub, currency: user.activePark.currency }}
          attention={sub._id === newlyEstablished}
        />
      ))}
      { !close && (
        <Form
...
export default inject('rootStore')(observer(Subscriptions));
